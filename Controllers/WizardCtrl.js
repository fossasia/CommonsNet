          ...
          if (vm.paymentfieldyes ==='yes') {
           ...
           result  = result.replace("PAYMENT_FIELD", "The owner declares that Wifi connection is completely free of any charge.");
            ...
            if (vm.specialdevices ==='yes') {
               ...
               var result  = result.replace('<text:p text:style-name="P155">SPECIAL_DEVICES</text:p>', ' ');
             ...
             if (vm.specialsettings ==='yes') {
               ...
               var result  = result.replace('<text:p text:style-name="P155">SPECIAL SETTINGS</text:p>', " ");
             ...
             if (vm.acceptterms ==='yes') {
               ...
               var result  = result.replace('<text:p text:style-name="P155">ACCEPT_TERMS</text:p>', " ");
            ...
            if (vm.socialprofile ==='yes') {
               ...
               var result  = result.replace('<text:p text:style-name="P155">LIKE</text:p>', " ");


                  } 
            if (vm.downloading ==='yes') {
                var result  = result.replace("DOWNLOAD", 'download terms of use pdf file');
                  }
            else {
               var result  = result.replace('<text:p text:style-name="P155">DOWNLOAD</text:p>', " ");

                  } 
            if (vm.register === 'yes') {
              var result = result.replace("REGISTER", 'register to website to use Wifi')
            }
            else {
              var result = result.replace('<text:p text:style-name="P155">REGISTER</text:p>', ' ')
            }
            if (vm.newsletter === 'yes') {
              var result = result.replace("NEWSLETTER", 'sign up to a newsletter')
            }
            else {
              result = result.replace('<text:p text:style-name="P155">NEWSLETTER</text:p>', '')
            }
            if (vm.mobilenumber === 'yes') {
              var result = result.replace("MOBILE_NUMER", 'give a mobile number')
            }
            else  {
